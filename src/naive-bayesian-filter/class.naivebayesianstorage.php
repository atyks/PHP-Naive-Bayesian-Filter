<?php
	...
	class NaiveBayesianStorage {
		...
		function getWord($word, $category_id) {
			...
			$rs = $this->con->select("SELECT * FROM nb_wordfreqs WHERE word='" . $this->con->escapeStr($word) . "' AND category_id='" . $this->con->escapeStr($category_id) . "'");

			if ($rs->isEmpty()) {
				$details['count'] = 0;
			}
			else {
				$details['count'] = $rs->f('count');
			}
		...
		function updateProbabilities() {
			...
			while (!$rs->EOF()) {
				...
				$this->con->execute("UPDATE nb_categories SET word_count=" . (int) $rs->f('total') . ", probability=" . $proba . " WHERE category_id = '" . $rs->f('category_id') . "'");
				$rs->moveNext();
			}
		...
		function saveReference($doc_id, $category_id, $content) {
			...
			return $this->con->execute("INSERT INTO nb_references (id, category_id, content) VALUES ('" . $this->con->escapeStr($doc_id) . "', '" . $this->con->escapeStr($category_id) . "', '" . $this->con->escapeStr($content) . "')");
		}
